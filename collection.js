/* ============================================================
   ...
   ------------------------------------------------------------
   Loaded as a classic <script> BEFORE app.js / stats.js, so it
   publishes a single global `Collection`. Everything about
   *reading the sheet* lives here — the CSV source, the column
   names, the blank-cell fallbacks, and the parsing that turns a
   raw CSV row into a disc object. Anything about *showing* a
   disc (colors, covers, layout) stays in the page that shows it —
   the two DOM helpers here (el, escapeHtml) are the exception,
   because both pages build elements and neither should own them.

   Two pages consume this: index.html (the browsable grid) and
   stats.html (the infographic breakdown). They agree on what a
   disc *is* precisely because they both read it from here.

   Requires PapaParse to be loaded first. No build step.
   ============================================================ */
window.Collection = (function () {
  ...
  // Only what the pages actually consume. Everything else above is an
  // implementation detail of turning a CSV row into a disc — exporting it would
  // freeze internals into an API that nothing calls.
  return {
    CONFIG,
    el,
    escapeHtml,
