/* ============================================================
   ...
   Requires collection.js (and therefore PapaParse) first.
   ============================================================ */
(function () {
  'use strict';
  ...
  // <tag class="cls">text</tag>, shared with app.js — this file makes a lot.
  const { el } = Collection;
  ...
  async function init() {
    ...
    try {
      const discs = await Collection.load();
      if (discs.length === 0) {
  ...
  function gridUrl(query) {
    return `index.html?${Collection.usingSample() ? 'sample&' : ''}${query}`;
  }
